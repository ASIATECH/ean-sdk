...
//itineraryId
  ...
  function sendOrder(data, callback) {
  	...
  	url = config.ean.bookReservation+"?"+qs.stringify(data)+"&"+qs.stringify(config.settings)
    console.log(url);
...
// confirmationNumber
  function cancelOrder(data, callback) {
    url = config.ean.cancelReservation+"?"+qs.stringify(data)+"&"+qs.stringify(config.settings)
    console.log(url);
